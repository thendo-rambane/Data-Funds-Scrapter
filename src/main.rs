...
fn get_trusts(url: &str) -> Vec<UnitTrust> {
    ...
    unit_trusts
        ...
        .map(|link| {
            ...
            let details_document = get_document(&fund_details_url).unwrap();
            let returns_document = get_document(&performance_url).unwrap();
            let info = get_detailed_information(&details_document);
            let fees = get_fees_and_costs(&details_document);
            let statutory_info = get_statutory_data(&details_document);
            let returns = get_returns(&returns_document);
